...
# modded by @zeticsce
# meta developer: таких нет, нум спизжен.
...
class NumMod(loader.Module):
    ...
    async def numfiltercmd(self, message):
        ...
        if not args:
            return await utils.answer(
                ...
                f"Игнор регистра(что за залупа объясните)!!"
            )
    ...
    async def watcher(self, message):
        ...
        if send_mesа := re.search(
                r"(?P<z>бей\s|кусь\s|кусай\s|заразить\s|еби\s|уеби\s{,2}\s)(?P<lvl>[1-9]?[0]?\s)?(?P<link>@[0-9a-z_]+|(?:https?://)?t\.me/[0-9a-z_]+|tg://openmessage\?user_id=(?P<id>[0-9]+))",
                text):
        ...
        elif send_mesb := re.search(r"(?P<sb>безопасность\s)(?P<text>)", text):
        ...
        elif send_mesip := re.search(r"(?P<ipat>пат\s)(?P<lvl>[0-5]+)", text):
            send_mesip = send_mesip.groupdict()
            send_mesip['ipat'] = '.имя патогена '
            send_mesip['lvl'] = send_mesip['lvl'] or ''
